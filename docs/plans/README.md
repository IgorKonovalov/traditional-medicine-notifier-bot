...
- **007 — Navigation shell** is the foundation: persistent reply-keyboard menu,
  ...
  kit, settings hub, onboarding refresh, and the `FEATURE_COMBINATIONS_BROWSER`
  flag. **Must land first** — 008 and 009 build on its kit.
...
- **009 — Library browser** unifies herbs/tips/guides under `📚 Библиотека` with
  rich cross-linked cards; the combinations browser is built but **gated** behind
  the flag (ADR 006 doctor-gate).
