...
| `content/guides/*` (long-form articles) | ✅ | **guide** pull content type (ADR 008, Plan 006): a markdown body split into ordered sections on `##` headings; optional `TipSource` citation; 8 Tibetan guides authored — flagship «Основы тибетской медицины», сезонное питание, распорядок дня, суточный ритм, the Plan 016 Wave 1 theory pair «Шесть вкусов» + «Как возникает болезнь», and the Plan 013 pair «Три природы человека» (constitution portraits) + «Как врач наблюдает тело» (diagnosis observation). Pulled, not pushed — exempt from the proactive budget |
