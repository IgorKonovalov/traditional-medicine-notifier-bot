...
| Admin commands (`/stats`) | ✅ | `bot/commands/stats.ts` — admin-gated (`adminTelegramIds`) plaintext readout: user counts · reminders · proactive pushes · donations · version (Plan 032). Composes four global aggregate repo queries (`getUserStats`/`getReminderStats`/`getProactiveStats`/`getDonationTotals`, each owning its table per ADR 003). Inline English (operator-only, out of `messages.ts`); deliberately unlisted — not in `setBotCommands`, `/help`, the menu, or the router; non-admins/channel posts/empty allowlist get no reply |
