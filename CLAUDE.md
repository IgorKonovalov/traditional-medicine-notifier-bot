...
## Package manager is pnpm (ADR 016)

- **Use `pnpm` / `pnpm run …`, never `npm`/`npx`.** The project is pinned to
  `pnpm@11.x` via the `packageManager` field (Corepack) with a committed
  `pnpm-lock.yaml`; a bare `npm install` would ignore the lockfile and the
  cooldown below.
- **7-day supply-chain cooldown.** `pnpm-workspace.yaml` sets
  `minimumReleaseAge: 10080`, so `pnpm add`/`pnpm update` won't resolve any
  version published less than 7 days ago. Frozen-lockfile installs (CI/Docker)
  are unaffected. Urgent-patch bypass and rationale: ADR 016.
- **Build scripts are default-denied** (`allowBuilds` in `pnpm-workspace.yaml`).
  Adding a dependency that runs an install/build script means adding it there on
  purpose.
...
- **Content index at `content/.index/`** is a generated, committed cross-file lookup (`herbs.json`, `combinations.json`, `categories.json`, `tips.json`, `guides.json`, `foods.json`). Regenerated by `pnpm run content:index`; the pre-commit hook keeps it in sync, CI guards against drift via `content:index:check`. Categories are generic (ADR 007): a `Category` can group herbs **and** combinations (`combinationCount`); a combination may carry an optional `category` (e.g. `rinchen-pills`) and a `nature` («Сущность») value.
...
- **The 🌿 herb branch is *labelled* «Ингредиенты» (Plan 019)** — same
  ...
  binomials by `pnpm run content:members` (map: `research/ingredient-member-map.json`).
