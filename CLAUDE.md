...
- **Informational, not medical advice.** This bot is an educational reference about traditional-medicine practices. It must never present content as diagnosis or treatment. Every herb page ends with the standard disclaimer (see `src/bot/messages.ts → disclaimer`); `/start` and `/help` surface it too. When authoring or rendering content, keep the framing descriptive ("traditionally used for…"), never prescriptive.
- **Content index at `content/.index/`** is a generated, committed cross-file lookup (`herbs.json`, `categories.json`, `tips.json`). Regenerated by `npm run content:index`; the pre-commit hook keeps it in sync, CI guards against drift via `content:index:check`.
